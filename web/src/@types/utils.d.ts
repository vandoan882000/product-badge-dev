...
declare global {
  ...
  declare type FunctionDetail<F extends AnyFunction> = F extends (...args: infer Params) => infer ReturnType
    ? [Params, ReturnType]
    : never;
  ...
  declare type IsBuiltIn = Primitive | AnyFunction | Date | Error | RegExp;
  ...
  declare type DeepPartial<T> = T extends AnyFunction
    ? T
    : T extends object
    ? {
  ...
  declare type UnionToIntersection<U> = (U extends any ? (k: U) => void : never) extends (k: infer I) => void
    ? I
    : never;
  ...
  declare type ObjectValuesOf<T extends AnyObject> = Exclude<Exclude<Extract<ValuesOf<T>, object>, never>, Array<any>>;
  ...
  declare type ToRequiredKeys<T, RK extends keyof T> = Omit<T, RK> & Required<Pick<T, RK>>;
  declare type ToPartialKeys<T, RK extends keyof T> = Omit<T, RK> & Partial<Pick<T, RK>>;

  type EntriesType = [PropertyKey, any][] | ReadonlyArray<readonly [PropertyKey, any]>;
  type UnionObjectFromArrayOfPairs<Entries extends EntriesType> = DeepWritable<Entries> extends (infer R)[]
  ...
  type EntriesToObject<Entries extends EntriesType> = MergeIntersectingObjects<
    UnionToIntersection<UnionObjectFromArrayOfPairs<Entries>>
  >;
